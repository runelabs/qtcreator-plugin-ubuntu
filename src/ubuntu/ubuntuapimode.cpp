    ...
    UbuntuWebMode(parent)
{
    ...
    QFile offlineDocs(QLatin1String(Ubuntu::Constants::UBUNTU_API_OFFLINE));
    if (offlineDocs.exists()) {
        setUrl(QUrl::fromUserInput(QLatin1String(Ubuntu::Constants::UBUNTU_API_OFFLINE)));
    } else {
        setUrl(QUrl::fromUserInput(QLatin1String(Ubuntu::Constants::UBUNTU_API_ONLINE)));
    }
}
