...
void UbuntuPackagingWidget::onFinishedAction(QString cmd) {
    ...
    } else if ((cmd == QString::fromLatin1(Constants::UBUNTUPACKAGINGWIDGET_ONFINISHED_ACTION_CLICK_CMAKECREATE).arg(Ubuntu::Constants::UBUNTU_SCRIPTPATH)) && (ui->pushButtonReviewersTools->isVisible()) ) {
...
void UbuntuPackagingWidget::on_pushButtonClickPackage_clicked() {
    ...
    Core::Command *cmd = 0;
    if(ProjectExplorer::ProjectExplorerPlugin::currentProject()->projectManager()->mimeType()
            == QLatin1String(CMakeProjectManager::Constants::CMAKEMIMETYPE)) {
        cmd = Core::ActionManager::instance()->command(Core::Id(Constants::UBUNTUPACKAGINGWIDGET_BUILDCMAKEPACKAGE_ID));
    } else {
        cmd = Core::ActionManager::instance()->command(Core::Id(Constants::UBUNTUPACKAGINGWIDGET_BUILDPACKAGE_ID));
    }
